From Stdlib Require Import FMapInterface
  FMapFacts FMapAVL OrderedType FunInd.
Module Type S(X:DecidableType).
  ...
  Parameter multiplicity : A -> t -> option nat. 

(*   Parameter In: A -> t -> Prop. *)
...
(*   Parameter In_mem: forall a ms, In a ms <-> mem a ms = true. *)
  
  Parameter is_empty_no_mem : forall ms, is_empty ms = true <-> (forall a, mem a ms = false). 
(*   Parameter is_empty_no_In : forall ms, is_empty ms = true <-> (forall a, ~mem a ms).  *)
...
(*   Parameter add_In: forall a b ms, X.eq a b -> In a (add b ms). *)
  ...
  Parameter mem_add_is_mem : forall a b ms, mem a ms = true -> mem a (add b ms) = true.
