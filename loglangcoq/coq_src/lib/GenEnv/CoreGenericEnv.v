...
From Stdlib Require Import Utf8 Equalities List.
...
(** Push and pop the head binding. *)
Parameter push : TVar -> A -> gen_env A -> gen_env A.
Parameter pop : gen_env A -> option ((TVar * A) * gen_env A).
...
Axiom push_concat : forall A x (v : A) (E : gen_env A),
  push x v E = E & (x ∶ v).
Axiom pop_push : forall A x (v : A) (E : gen_env A),
  pop (push x v E) = Some ((x, v), E).

Axiom pop_None : forall A,
  pop (@empty A) = None.
Axiom pop_None_inv : forall A (E : gen_env A),
  pop E = None ->
  E = (@empty A).

Axiom pop_Some : forall A x (v : A) (E : gen_env A),
  E <> (@empty A) ->
  exists x, exists (v : A), exists (E' : gen_env A),
    pop E = Some ((x, v), E').
Axiom pop_Some_inv : forall A x (v : A) (E E' : gen_env A),
  pop E = Some ((x, v), E') ->
  E = push x v E'.
