...
(* ---------------------------------------------------------------------- *)
(** **** Properties of push and pop *)

Lemma push_concat : forall A x (v : A) (E : gen_env A),
  push x v E = E & (x ∶ v).
Proof. auto. Qed.

Lemma pop_push : forall A x (v : A) (E : gen_env A),
  pop (push x v E) = Some ((x, v), E).
Proof. auto. Qed.

Lemma pop_None : forall A,
  pop (@empty A) = None.
Proof. auto. Qed.

Lemma pop_None_inv : forall A (E : gen_env A),
  pop E = None ->
  E = (@empty A).
Proof.
  intros ? E Hnone.
  induction E as [ | (x, v) E IHE ]; inversion Hnone; auto.
Qed.

Lemma pop_Some : forall A x (v : A) (E : gen_env A),
  E <> (@empty A) ->
  exists x, exists (v : A), exists (E' : gen_env A),
    pop E = Some ((x, v), E').
Proof.
  intros ? x v E Hunempty.
  induction E as [ | (x', v') E' IHE' ]. elim Hunempty; auto.
  exists x'; exists v'; exists E'; auto.
Qed.

Lemma pop_Some_inv : forall A x (v : A) (E E' : gen_env A),
  pop E = Some ((x, v), E') ->
  E = push x v E'.
Proof.
  intros ? x v E E' Hsome.
  induction E as [ | (x', v') E'' IHE'' ]; inversion Hsome; auto.
Qed.
...
Proof.
  ...
  right; auto. simpl. destruct (eq_keys_dec x y); auto.
  left; exists w; auto.
Qed.
...
Proof.
  ...
  assert (ok (E ∖ {x})); auto. apply ok_remove; auto.
Qed.
